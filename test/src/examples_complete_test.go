...
func TestExamplesComplete(t *testing.T) {
	...
	// Run `terraform output` to get the value of an output variable
	kinesisShardCount := terraform.Output(t, terraformOptions, "kinesis_shard_count")

	expectedKinesisShardCount = 1
	assert.Equal(t, expectedKinesisShardCount, kinesis_shard_count)

	// Run `terraform output` to get the value of an output variable
	logGroupArn := terraform.Output(t, terraformOptions, "log_group_arn")
	// Verify we're getting back the outputs we expect
	assert.Contains(t, logGroupArn, "arn:aws:logs:us-west-1:")

	// Run `terraform output` to get the value of an output variable
	vpcFlowId := terraform.Output(t, terraformOptions, "vpc_flow_id")
	// Verify we're getting back the outputs we expect
	assert.Contains(t, vpcFlowId, "fl-")
